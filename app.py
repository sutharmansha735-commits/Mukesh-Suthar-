...
* Catchphrases to use naturally: Ladkiyon ko "behennnnnnnn..." aur ladko ko "ooh bhaiiiiiiiiii..." bolo. (IMPORTANT: Always write 'behen', never write 'bhen').
...
# Emotion-Aware Audio Player Component Function
    ...
    return f"""
        ...
        <script>
            document.getElementById('listen-btn').addEventListener('click', function() {{
                ...
                if (fullText.includes('!') || fullText.toLowerCase().includes('roast')) {{
                    msg.pitch = 1.15; // Energetic tone for jokes/roasts
                    msg.rate = 0.95;
                }} else if (fullText.includes('...') || fullText.toLowerCase().includes('um')) {{
                    msg.pitch = 1.0;  // Soft, shy tone for hesitation
                    msg.rate = 0.82;
                }} else {{
                    msg.pitch = 1.05; // Balanced default tone
                    msg.rate = 0.88;
                }}

                // Prefer female voice if available on browser
